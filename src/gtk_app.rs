...
impl AppData {
    pub fn new() -> Self {
        ...
        let menu_weak = SendWeakRef::from(menu.downgrade());
        gtk_app.connect_startup(move |gtk_app| {
            ...
            if let Some(window) = window_weak.upgrade() {
                ...
                #[cfg(target_os = "linux")]
                {
                    ...
                    main_menu.set_menu_model(&data.menu);
...
impl app::Application for GlideGTKApp {
    fn enter_fullscreen(&self) {
        ...
        #[cfg(not(target_os = "macos"))]
        {
            ...
            *INHIBIT_COOKIE.lock().unwrap() = Some(self.data.app.inhibit(window, flags, Some("Glide full-screen")));
        }
    ...
    fn set_video_renderer(&self, renderer: &video_renderer::VideoRenderer) {
        let video_area = renderer.as_gtk_widget().unwrap();
